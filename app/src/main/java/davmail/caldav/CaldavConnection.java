...
import com.google.code.cantilever.javax.xml.stream.XMLStreamConstants;
import com.google.code.cantilever.javax.xml.stream.XMLStreamException;
import com.google.code.cantilever.javax.xml.stream.XMLStreamReader;
...
public class CaldavConnection extends AbstractConnection {
    ...
    protected final Logger wireLogger = LoggerFactory.getLogger(this.getClass());
    ...
    protected void setSocketTimeout(String keepAliveValue) throws IOException {
        if (keepAliveValue != null && keepAliveValue.length() > 0) {
          ...
          //XXX - UI Listner            DavGatewayTray.debug(new BundleMessage("LOG_SET_SOCKET_TIMEOUT", keepAlive));
        }
    }
    ...
    public void run() {
        ...
        try {
            while (!closed) {
                ...
                os.flush();
              //XXX - UI Listner                DavGatewayTray.resetIcon();
            }
        } catch (SocketTimeoutException e) {
        	//XXX - UI Listner            DavGatewayTray.debug(new BundleMessage("LOG_CLOSE_CONNECTION_ON_TIMEOUT"));
        } catch (SocketException e) {
        	//XXX - UI Listner            DavGatewayTray.debug(new BundleMessage("LOG_CONNECTION_CLOSED"));
        } catch (Exception e) {
            if (!(e instanceof HttpNotFoundException)) {
            	//XXX - UI Listner                DavGatewayTray.log(e);
            }
            try {
                sendErr(e);
            } catch (IOException e2) {
            	//XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_EXCEPTION_SENDING_ERROR_TO_CLIENT"), e2);
            }
        } finally {
            close();
        }
...
//            }
    ...
    private void appendContactsResponses(CaldavResponse response, CaldavRequest request, List<ExchangeSession.Contact> contacts) throws IOException {
        ...
        int count = 0;
        for (ExchangeSession.Contact contact : contacts) {
        	//XXX - UI Listner            DavGatewayTray.debug(new BundleMessage("LOG_LISTING_ITEM", ++count, size));
        	//XXX - UI Listner            DavGatewayTray.switchIcon();
    ...
    public void appendInbox(CaldavResponse response, CaldavRequest request, String subFolder) throws IOException {
        ...
        if (!session.isSharedFolder(folderPath)) {
            try {
            	...
            	//XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_ACCESS_FORBIDDEN", folderPath, e.getMessage()));
            }
        }
        response.startResponse(encodePath(request, request.getPath(subFolder)));
        response.startPropstat();
    ...
    public void sendInbox(CaldavRequest request) throws IOException {
        ...
        if (!session.isSharedFolder(request.getFolderPath(null)) && request.getDepth() == 1
                && !request.isLightning()) {
            try {
            	//XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_SEARCHING_CALENDAR_MESSAGES"));
                List<ExchangeSession.Event> events = session.getEventMessages(request.getFolderPath());
              //XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_FOUND_CALENDAR_MESSAGES", events.size()));
                appendEventsResponses(response, request, events);
            } catch (HttpException e) {
                // unauthorized access, probably an inbox on shared calendar
            	//XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_ACCESS_FORBIDDEN", request.getFolderPath(), e.getMessage()));
            }
        }
    ...
    public void reportItems(CaldavRequest request) throws IOException {
        ...
        if (request.isMultiGet()) {
            ...
            for (String href : request.getHrefs()) {
            	//XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_REPORT_ITEM", ++count, total));
              //XXX - UI Listner                DavGatewayTray.switchIcon();
                ...
                try {
                  ...
                  //XXX - UI Listner                    DavGatewayTray.warn(new BundleMessage("LOG_ITEM_NOT_AVAILABLE", eventName, href));
                    notFound.add(href);
                }
            }
    ...
    public void sendRoot(CaldavRequest request) throws IOException {
        ...
        if (request.depth == 1) {
            ...
            if (request.hasProperty("getctag")) {
                ExchangeSession.Folder rootFolder = session.getFolder("");
                response.appendProperty("CS:getctag", "CS=\"http://calendarserver.org/ns/\"", base64Encode(rootFolder.ctag));
            }
    ...
    public void sendUnsupported(CaldavRequest request) throws IOException {
      ...
      //XXX - UI Listner        DavGatewayTray.error(message);
        ...
        protected void buildDepth() {
            ...
            if ("infinity".equalsIgnoreCase(depthValue)) {
                ...
                try {
                	...
                	//XXX - UI Listner                    DavGatewayTray.warn(new BundleMessage("LOG_INVALID_DEPTH", depthValue));
                }
            }
        }
        ...
        protected void parseXmlBody() throws IOException {
            ...
            try {
                ...
                try {
                	...
                	//XXX - UI Listner                    DavGatewayTray.error(e);
                }
            }
        }
