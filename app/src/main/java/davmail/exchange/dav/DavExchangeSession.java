...
public class DavExchangeSession extends ExchangeSession {
    ...
    public String getAliasFromMailboxDisplayName() {
        ...
        try {
            ...
            if (rootFolder == null) {
                LOGGER.warn(new BundleMessage("EXCEPTION_UNABLE_TO_GET_MAIL_FOLDER", mailPath).format());
            } else {
    ...
    protected String getMailboxPath() {
        ...
        if (index >= 0 && mailPath.endsWith("/")) {
            ...
            LOGGER.warn(new BundleMessage("EXCEPTION_INVALID_MAIL_PATH", mailPath).format());
            return null;
        }
    }
    ...
    protected MultiStatusResponse[] searchItems(String folderPath, Set<String> attributes, Condition condition,
      ...
      //XXX - UI Listner        DavGatewayTray.debug(new BundleMessage("LOG_SEARCH_QUERY", searchRequest));
        MultiStatusResponse[] responses = DavGatewayHttpClientFacade.executeSearchMethod(
                httpClient, encodeAndFixUrl(folderUrl), searchRequest.toString(), maxCount);
      //XXX - UI Listner        DavGatewayTray.debug(new BundleMessage("LOG_SEARCH_RESULT", responses.length));
        return responses;
    }
            ...
            inputStream = new FilterInputStream(inputStream) {
                ...
                public int read(byte[] buffer, int offset, int length) throws IOException {
                    ...
                    if (totalCount - lastLogCount > 1024 * 128) {
                    	//XXX - UI Listner	DavGatewayTray.debug(new BundleMessage("LOG_DOWNLOAD_PROGRESS", String.valueOf(totalCount / 1024), method.getURI()));
