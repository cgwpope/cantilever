...
public abstract class AbstractServer extends Thread {
            ...
            try {
                ...
                if (keyStoreInputStream != null) {
                    try {
                    	...
                    	//XXX - UI Listner                        DavGatewayTray.warn(new BundleMessage("LOG_EXCEPTION_CLOSING_KEYSTORE_INPUT_STREAM"), exc);
                    }
                }
            }
        }
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
              ...
              //XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_CONNECTION_FROM", clientSocket.getInetAddress(), port));
                ...
                if (Settings.getBooleanProperty("davmail.allowRemote") ||
                    ...
                    clientSocket.close();
                  //XXX - UI Listner                    DavGatewayTray.warn(new BundleMessage("LOG_EXTERNAL_CONNECTION_REFUSED"));
                }
            }
        } catch (IOException e) {
            // do not warn if exception on socket close (gateway restart)
            if (!serverSocket.isClosed()) {
            	//XXX - UI Listner                DavGatewayTray.warn(new BundleMessage("LOG_EXCEPTION_LISTENING_FOR_CONNECTIONS"), e);
            }
        } finally {
            try {
            	...
            	//XXX - UI Listner                DavGatewayTray.warn(new BundleMessage("LOG_EXCEPTION_CLOSING_CLIENT_SOCKET"), e);
            }
            if (connection != null) {
                connection.close();
            }
    ...
    public void close() {
        try {
        	...
        	//XXX - UI Listner            DavGatewayTray.warn(new BundleMessage("LOG_EXCEPTION_CLOSING_SERVER_SOCKET"), e);
        }
    }
}
