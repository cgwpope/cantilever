...
public final class DavGatewayHttpClientFacade {
    static final Logger LOGGER = LoggerFactory.getLogger("davmail.http.DavGatewayHttpClientFacade");
    ...
    public static HttpMethod executeFollowRedirects(HttpClient httpClient, HttpMethod method) throws IOException {
        ...
        try {
        	//XXX - UI Listner            DavGatewayTray.debug(new BundleMessage("LOG_EXECUTE_FOLLOW_REDIRECTS", currentMethod.getURI()));
            ...
            while (redirectCount++ < 10
              ...
              //XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_EXECUTE_FOLLOW_REDIRECTS_COUNT", currentMethod.getURI(), redirectCount));
                httpClient.executeMethod(currentMethod);
                checkNTLM(httpClient, currentMethod);
