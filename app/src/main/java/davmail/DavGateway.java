...
public final class DavGateway {
    private static final Logger LOGGER = LoggerFactory.getLogger(DavGateway.class);
    ...
    public static void main(String[] args) {
        ...
        Settings.load();
      //XXX - UI Listner        DavGatewayTray.init();
        ...
        if (Settings.getBooleanProperty("davmail.server")) {
            Runtime.getRuntime().addShutdownHook(new Thread("Shutdown") {
                @Override
                public void run() {
                	//XXX - UI Listner                    DavGatewayTray.debug(new BundleMessage("LOG_GATEWAY_INTERRUPTED"));
                    DavGateway.stop();
                    stopped = true;
                }
            });
            ...
            try {
            	...
            	//XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_GATEWAY_INTERRUPTED"));
                stop();
              //XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_GATEWAY_STOP"));
            }
...
//        }
        ...
        if (showStartupBanner) {
        	//XXX - UI Listner            DavGatewayTray.info(new BundleMessage("LOG_DAVMAIL_GATEWAY_LISTENING", currentVersion == null ? "" : currentVersion, messages));
        }
        if (!errorMessages.isEmpty()) {
        	//XXX - UI Listner            DavGatewayTray.error(new BundleMessage("LOG_MESSAGE", errorMessages));
        }
        ...
        new Thread("CheckRelease") {
            ...
            public void run() {
                ...
                if (currentVersion != null && currentVersion.length() > 0 && releasedVersion != null && currentVersion.compareTo(releasedVersion) < 0) {
                	//XXX - UI Listner                    DavGatewayTray.info(new BundleMessage("LOG_NEW_VERSION_AVAILABLE", releasedVersion));
                }
    ...
    public static void stop() {
      ...
      //XXX - UI Listner        DavGatewayTray.info(new BundleMessage("LOG_GATEWAY_STOP"));
      //XXX - UI Listner        DavGatewayTray.dispose();
    }
    ...
    private static void stopServers() {
        for (AbstractServer server : SERVER_LIST) {
            ...
            try {
            	...
            	//XXX - UI Listner                DavGatewayTray.warn(new BundleMessage("LOG_EXCEPTION_WAITING_SERVER_THREAD_DIE"), e);
            }
        }
    }
    ...
    public static String getReleasedVersion() {
        ...
        if (!Settings.getBooleanProperty("davmail.disableUpdateCheck")) {
            ...
            try {
            	...
            	//XXX - UI Listner                DavGatewayTray.debug(new BundleMessage("LOG_UNABLE_TO_GET_RELEASED_VERSION"));
            } finally {
